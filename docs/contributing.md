...
- 🐛 Report bugs:
  ...
  Use the _🐛 Bug report_ template at <https://github.com/munich-quantum-toolkit/core-plugins-catalyst/issues>.
...
- 💡 Propose features:
  ...
  Use the _✨ Feature request_ template at <https://github.com/munich-quantum-toolkit/core-plugins-catalyst/issues>.
...
- 🙌 Community support:
  ...
  <https://github.com/munich-quantum-toolkit/core-plugins-catalyst/discussions>.
...
It will also collect code coverage information and upload it to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/core-plugins-catalyst).
...
:::{tip}
If you want to disable configuring and building the C++ tests, you can pass {code}`-DBUILD_MQT_CORE-PLUGINS-CATALYST_TESTS=OFF` to the CMake configure step.
:::
...
The Python package itself lives in the {code}`python/mqt/core-plugins-catalyst` directory.

The package lives in the {code}`src/mqt/core-plugins-catalyst` directory.
...
MQT core-plugins-catalyst adheres to [Semantic Versioning], with the exception that minor releases may include breaking changes.
...
Use it to explain how users may need to adapt their usage of MQT core-plugins-catalyst, highlight new workflows, or clarify the impact of important updates.
...
When it is time to release a new version of MQT core-plugins-catalyst, create a PR that prepares the release.
This PR should:
...
This does not mean that the dependency update itself is a breaking change for MQT core-plugins-catalyst.
If you are sure that the dependency update does not introduce any breaking changes for MQT core-plugins-catalyst, you can remove the {code}`minor` or {code}`major` label from the PR.
...
Afterward, navigate to the [Releases page](https://github.com/munich-quantum-toolkit/core-plugins-catalyst/releases) on GitHub, edit the release draft if necessary, and publish the release.

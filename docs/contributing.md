...
[GitHub Actions page](https://github.com/munich-quantum-toolkit/core-plugins-catalyst/actions/workflows/ci.yml).
...
you can pass {code}`-DBUILD_MQT_CORE-PLUGINS-CATALYST_TESTS=OFF` to the CMake
configure step.
:::
...
pass {code}`-DBUILD_MQT_CORE-PLUGINS-CATALYST_BINDINGS=ON` to the CMake configure
step.
...
The Python package itself lives in the {code}`python/mqt/core-plugins-catalyst`
directory.
...
{code}`python/mqt/core-plugins-catalyst` directory.
...
MQT core-plugins-catalyst adheres to [Semantic Versioning],
...
Use it to explain how users may need to adapt their usage of MQT core-plugins-catalyst,
...
When it is time to release a new version of MQT core-plugins-catalyst,
...
for MQT core-plugins-catalyst.
If you are sure that the dependency update does not introduce any breaking
changes for MQT core-plugins-catalyst, you can remove the {code}`minor` or {code}`major`
label from the PR.

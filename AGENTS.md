...
- Install package without build isolation (fast rebuilds): `uv sync --inexact --no-dev --no-build-isolation-package mqt-core-plugins-catalyst`
...
- NEVER edit `.pyi` files in `python/mqt/core-plugins-catalyst/` manually; they are auto-generated by nanobind stubgen.
